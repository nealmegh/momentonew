        ...
        <div class="hotel-list">
            <div class="row image-box hotel listing-style1">
                @foreach($tours as $tour)
                    <?php  $image = $tour->images()->where('type', 'gallery')->first(); ?>
                    <div class="col-sm-6 col-md-4">
                        <article class="box">
                            <figure>
                                <a href="{{ url('tours/detail/'.$tour->id) }}" class="hover-effect popup-gallery">
                                    @if($image != null)
                                        <img width="270" height="161" style="margin: 0 auto;" alt="" src="{{ url($image->path.'/'.$image->name) }}">
                                    @else
                                        <img width="270" style="width: 161px; margin: 0 auto;" alt="" src="{{ url('images/no-image.png') }}">
                                    @endif
                                </a>
                            </figure>
                            <div class="details">
                                ...
                                <div class="feedback">
                                    <div data-placement="bottom" data-toggle="tooltip" class="five-stars-container" title="5 stars">
                                        <span style="width: {{ $tour->ratings()->avg('rating_value')*20 }}%;" class="five-stars"></span>
                                    </div>
            ...
            @if($type == 'all-tours')
                {!! $tours->render() !!}
            @elseif($type == 'result')
                {!! $tours->appends(Input::only('placeOrHotel', 'date_from', 'date_to', 'num_of_room', 'num_of_adult', 'num_of_child'))->render() !!}
            @endif
        </div>
    {{--@endif--}}
