...
@section('main-content')
    ...
    @if($cars != null)
        <div class="hotel-list">
                        ...
                        <article class="box">
                            <figure>
                                <a href="ajax/slideshow-popup.html" class="hover-effect popup-gallery">
                                    <img width="270" height="161" alt="" src="{{  url($car->images[2]->path .'/'.$car->images[2]->name ) }}">
                                </a>
                            </figure>
                            <div class="details">
                        <span class="price">
                            ...
                            {{ $car->price_per_adult }}
                                ...
                                <h4 class="box-title">{{ $car->name }}<small>{{ $car->city.', '.$car->country }}</small></h4>
                                <div class="feedback">
                                ...
                                <p class="description">{!! substr($car->general_information, 0,125) !!}</p>
                                <div class="action">
                                    {!! Form::open(['url' => 'cars/detail/'.$car->id, 'method' =>'get', 'class'=>'hotelDetail' ,'target' => '_blank']) !!}
                                    ...
                                    <a class="button btn-small yellow popup-map" href="#" data-box="{{ $car->google_map }}">VIEW ON MAP</a>
                                    {!! Form::close() !!}
            ...
            @if($type == 'allCars')
                {!! $cars->appends(Input::only('searchable'))->render(); !!}

            @elseif($type == 'tour')
                {!! $cars->appends(Input::only('searchable',  'placeOrHotel', 'date_from', 'date_to', 'num_of_room', 'num_of_adult', 'num_of_child'))->render(); !!}
            @endif
        </div>
    @endif
        {{--bipon end--}}
