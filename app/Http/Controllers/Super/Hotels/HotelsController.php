...
class HotelsController extends Controller {
    ...
    {
        $hotels = Hotel::all();
     //   $avg = $hotels[0]->roomTypes()->avg('price');
     //   dd($avg);
        $count = 0;
    ...
    {
        $hotelID = Input::get('hotel');
        $hotel = Hotel::findOrFail($hotelID);

        if($hotel->status == 1)
        {
             Hotel::where('id', $hotelID)->update(['status' => 0]);
        }
        else
        {
            Hotel::where('id', $hotelID)->update(['status' => 1]);
        }
...
//                 dd($availableRooms);
             ...
             */
            $similar_hotels = Hotel::where('city', '=', $hotel->city)->where('id', '!=', $hotel->id)->Limit(3)->get();
...
//        $hotelBookings = HotelBooking::where('hotel_id', '=', $hotel->id);
//
//        foreach($hotelBookings as $hotelBooking) {
//            $bookings = Booking::where('booking_no', '=', $hotelBooking->booking_no)->first();
//
//            if($bookings->route == 'hotel')
//            {
//                 $bookings->delete();
//            }
//        }
...
//        $id = $hotel->id;
//
//        $roomVacancy = Room::where('hotel_id', '=', $id );
//        $roomVacancy->delete();
//
//        $room = RoomType::where('hotel_id', '=', $id);
//
//        $room->delete();
//       // dd($hotel->images[0]->type);
//
...
//        }
//
//        if (file_exists('images/hotels/gallery/'.$hotel->id.'/thumbnail')) {
//            rmdir('images/hotels/gallery/'.$hotel->id.'/thumbnail');
//        }
//
//        if (file_exists('images/hotels/gallery/'.$hotel->id.'/thumbnail')) {
//            rmdir('images/hotels/gallery/'.$hotel->id);
//        }
