<?php namespace App\Http\Controllers\Super\Booking;
...
//                dd($booking->payment);
    ...
    {
//        dd($request->booking_no);
//        $booking_no = Booking::findOrFail($request->get('booking_no'));
        $booking = Booking::where('booking_no', '=', $request->booking_no)->first();
        if($booking->isEmpty) {
            return Redirect::back();
        }else {
            return redirect('hotels/booking/confirm/'.$booking->booking_no);
        }
    }
    ...
    public function complete($booking_no)
    {
    ...
    /**
     * Pin Code Email to Customer
     */
    public function pinEmail($no)
    {
        $booking = Booking::where('booking_no','=', $no)->first();
        $guestID = $booking->guest_id;
        $guest = Guest::where('id', '=', $guestID)->first();

        $mail_details = array('name'=>$guest->first_name, 'email'=> $guest->email, 'pin_code'=> $booking->pin_code, 'p_message'=>'Please use this pin code to complete your booking.');

        Mail::send('emails.pin_code_send', $mail_details, function($message) use ($mail_details)
        {
            $message->to($mail_details['email'], $mail_details['name'])->subject('Booking Pin Code from Momento');
        });

        Booking::where('booking_no', '=', $no)->update(array('pincode_email' => 1));
